...
| 김민수 | 여태 경험하지 못했던 프로젝트였다. 처음으로 해보는 데이터 베이스와 설계를 중점적으로 진행하는 프로젝트였기 때문에 현실적인 부분에서 약간은 벗어나 더 많은 아이디어와 기능에 대한 얘기를 나눌 수 있었다. 그러면서 자연스레 가지고 있는 데이터를 어떻게 활용할 수 있을지 고민을 하고 데이터 베이스를 구성하며 조인, 정규화, 제약 조건 등에 관한 이해를 할 수 있게 되었다. 프로젝트를 진행하면서 초기 구상과 달라지면 변경하기 어렵기 때문에 설계가 얼마나 중요한지, 우리의 핵심 주제가 무엇인지, 팀원들의 소통을 통해 팀원 모두가 같은 방향을 바라 보는지가 굉장히 중요하다는 것을 다시 깨닫게 되었다. |
