...
<details>
<summary>🩺 증상 및 사용자 증상 기록</summary>
...
<details>
<summary>⚠️ 약물 경고 및 상호작용</summary>
...
#### 💬 커뮤니티 기능 강화
게시판을 증상별, 반려동물 종류별 등으로 세분화하고
사진 업로드, 대댓글(답글) 기능 등 상호작용성을 높여
유저 간 정보 공유와 소통이 활발히 이뤄지도록 개선합니다.
...
| 이름 | 내용 |
| --- | --- |
| 안진기 | 다양한 경험을 쌓고 싶다는 마음으로 시작한 프로젝트였는데, 돌아보면 그 이상으로 의미 있는 시간이었습니다. 기획부터 요구사항 정리, DB 설계와 기능 구현, 마무리 발표까지 이어지는 여정 속에서 무언가를 함께 만들어간다는 감각이 좋았고, 팀원들과의 끊임없는 피드백과 수정 과정을 통해 더 나은 결과를 추구하려 노력했습니다. 다만, 후반에 기능을 추가하면서 ERD와 요구사항을 다시 손보는 일이 반복되어, 처음 기획을 좀 더 구체적으로 잡고 지켜나가는 것이 얼마나 중요한지를 크게 배웠습니다. 문서화와 개발을 병행하면서 체력적으로도 쉽지 않았지만, 그만큼 얻은 것도 많았습니다. 현실적인 제약 속에서도 좋은 결과물을 만들어낸 팀원들에게 감사하고, 이번 경험을 바탕으로 앞으로의 협업에서도 더 유연하고 단단하게 성장해가고 싶습니다.
| 김민수 | 여태 경험하지 못했던 프로젝트였습니다. 처음으로 해보는 데이터 베이스와 설계를 중점적으로 진행하는 프로젝트였기 때문에 현실적인 부분에서 약간은 벗어나 더 많은 아이디어와 기능에 대한 얘기를 나눌 수 있었습니다. 그러면서 자연스레 가지고 있는 데이터를 어떻게 활용할 수 있을지 고민을 하고 데이터 베이스를 구성하며 조인, 정규화, 제약 조건 등에 관한 이해를 할 수 있게 되었습니다. 프로젝트를 진행하면서 초기 구상과 달라지면 변경하기 어렵기 때문에 설계가 얼마나 중요한지, 우리의 핵심 주제가 무엇인지, 팀원들의 소통을 통해 팀원 모두가 같은 방향을 바라 보는지가 굉장히 중요하다는 것을 다시 깨닫게 되었습니다. 저보다 열심히 해준 팀원들 정말 수고 많으셨습니다. |
| 최정우 | 이번 프로젝트는 데이터베이스만을 활용해 기능을 직접 구현하는 방식으로 진행되었습니다. 서버나 백엔드 로직 없이, 오직 SQL과 저장 프로시저만으로 다양한 기능을 설계하는 방식이 처음에는 다소 어색하게 느껴졌습니다. 하지만 이번 경험을 통해, 데이터베이스가 단순한 저장소를 넘어 복잡한 로직을 처리할 수 있는 도구임을 깨닫게 되었습니다. 무엇보다도 이번 프로젝트를 통해 설계의 중요성을 절실히 체감했습니다. 처음에는 기능부터 생각하고, ERD는 나중에 맞춰 그릴 계획이었습니다. 하지만 실제로 테이블을 만들고 프로시저를 작성하는 과정에서, ERD 작성 시 오해하거나 놓친 부분들이 고스란히 오류로 이어졌습니다. 이 프로젝트를 통해 구현 이전의 설계가 부족하면, 구현 과정에서 반드시 대가를 치르게 된다는 교훈을 뼈저리게 느꼈습니다. 팀원들 다들 고생하셨습니다. |
| 박채연 | 팀원들이 많이 이끌어주셔 빠른 시간 이내 많은 것을 완료할 수 있었고 감사했습니다. 또한 스프링에서 jpa를 사용해 직접 sql문을 활용하는 일이 별로 없어 프로시져를 작성해보는 작업이 새로웠습니다. 내부적으로 sql문들이 어떻게 작성되어 스프링에서 메서드로 이용할 수 있는 것인지 이해하는 시간이 되어 흥미로웠고 이와 같은 점에서 배울 부분들이 있어 의미 있었습니다. |
| 이원진 | 처음 프로젝트를 진행하면서 배웠던 내용을 실제로 활용하는 것이 생각보다 많이 어렵다는 것을 느꼈습니다 특히 진행 과정 중 생기는 각종 오류들을 해결하는 것이 큰 난관이었습니다. 그럼에도 뛰어난 팀원분께 많은 도움을 받았고 덕분에 수업에서 배웠던 내용들을 복습하고 응용해보는 기회가 된것 같습니다. 이 경험들이 앞으로 진행할 프로젝트에도 도움이 되면 좋겠습니다. |
| 이승진 | SQL만으로 프로젝트를 진행한 건 처음이라 낯설었지만, ERD 설계부터 쿼리 작성, 프로시저 만들기까지 직접 시도해보며 자연스럽게 익혀갈 수 있었습니다. 데이터 흐름을 구조적으로 고민하고, 프로그래밍 없이 기능을 구현해보는 경험이 새로웠습니다. 좋은 팀원들과 함께해서 끝까지 의미있게 마무리할 수 있었습니다. |
